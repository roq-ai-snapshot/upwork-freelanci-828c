...
import { getUserSkills } from 'apiSdk/user-skills';
import { UserSkillInterface } from 'interfaces/user-skill';
import { getProjects } from 'apiSdk/projects';
import { ProjectInterface } from 'interfaces/project';

import { UserSkillListPage as ShowCaseList } from 'pages/user-skills';
...
function DashboardPage() {
  ...
  const userSkillStat = useSWR<PaginatedInterface<UserSkillInterface>>(
    () => `/user-skills`,
    () => getUserSkills({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'User Skills',
      result: userSkillStat?.data?.totalCount,
      isLoading: userSkillStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Projects',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`user-skills`} passHref legacyBehavior>
                  <Button
