...
import { getUserSkillById } from 'apiSdk/user-skills';
import { UserSkillInterface } from 'interfaces/user-skill';

function UserSkillViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('user_skill', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/user-skills/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('skill', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Skill"
                    text={
                      <Link as={NextLink} href={`/skills/view/${data?.skill?.id}`}>
                        {data?.skill?.name}
                      </Link>
                    }
                  />
